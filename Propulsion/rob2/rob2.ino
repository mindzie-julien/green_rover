...
#define ENCA 2//18 moteur M 21
#define ENCB 3//19 moteur M 20
...
char directionSense = 'F', turnSide = '0', msg = ' ', spd = ' ';
bool wait = false, taskProcessed = true, infiniteRide = false, stopMeasure = false, stopEncode = false;
char taskProcessing = NONE;
int target = 0,speedMotor = 0, countStep = 0, currentStepTime = 0, previousStepTime = 0, countStepDistance = 0;
...
void receiveEvent(int byteCount) {
    ...
    msg = received_data.charAt(1);
    number = (received_data.charAt(2) - '0') * 100 + (received_data.charAt(3) - '0') * 10 + (received_data.charAt(4) - '0');
    spd = received_data.charAt(5); 
    received_data = "";
    Serial.print(msg);
    Serial.print(number);
    Serial.print(spd);
...
void sendI2CMessage() {
  Wire.write(err);
  Serial.println("message envoyee");
  Serial.println(err);
}
...
void stop()
{
  ...
  int remainingError = target - pos;
  if (remainingError > 0){
    err = remainingError / 50;
    Serial.println(err);
    sendI2CMessage();
  }
  switch(speedMotor)
