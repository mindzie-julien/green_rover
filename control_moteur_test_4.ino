...
char request;    
     ...
     int ui=0;
     String command = "";
      int value = 0, numberTest = 0;
float angle = 0, stepRight = 0, stepLeft = 0;      
bool msgIsAviable =false, startDelay = true;      
byte requestIndex = 0;
...
char directionSense = 'F', turnSide = '0', msgFuture = ' ', msg =' ';
bool wait = false, taskProcessed = true, futureTaskProcessed = true, infiniteRide = false, stopMeasure = false, stopEncode = false;
...
void receiveEvent(int nBytes) {
    ...
    if(!error){ 
     ...
     if(doc.containsKey("angle"))
     {
      ...
      if( command == "R" )
      {
        msgFuture = TURN_RIGHT_5;
        targetGive = value*8;
        Serial.println("initMsg---RIGHT---target: "+(String)value+" targetGive: "+(String)targetGive);
      }
      if( command == "L" ){
        msgFuture = TURN_LEFT_5; 
        targetGive = value*8;
        Serial.println("initMsg---LEFT---target: "+(String)value+" targetGive: "+(String)targetGive);
      }
      if( command == "F" ){
        msgFuture = RIDE_1CM;
        targetGive = value*5;
        Serial.println("initMsg---FORWARD---target: "+(String)value+" targetGive: "+(String)targetGive);
      }
      if( command == "E" ){
        msgFuture = STOP;
        targetGive = 0;
        Serial.println("initMsg---STOP---target: "+(String)value+" targetGive: "+(String)targetGive);
      }
      speedMotor =102;
      if( command == "B" ){
        directionSense = 'B';
        ...
        //targetGive = value*5;
        speedMotor = 102;
        Serial.println("initMsg---BACK---target: "+(String)value+" targetGive: "+(String)targetGive);
      }
      else{
        directionSense = 'F';
      }
...
void readEncoder(){
  ...
  if(stopEncode == true)
    pos = 0;
 /*Serial.println(" enc");
 Serial.println("stepLeft: " + (String)stepLeft);
 Serial.println("stepRight: " + (String)stepRight);
...
void readEncoder_I(){
  ...
  if(b > 0){
    ...
    //angle = angle - 0.125;
    //Serial.println("pos_i: " + (String)pos_i);
    /*if(stepLeft > stepRight){
      angle = angle + 0.125;
    }
    else if(stepLeft < stepRight){
...
void turn()
{
      ...
      if(taskProcessing == TURN_RIGHT_5)//10 degree
      {
         ...
         if(directionSense == 'F')
         {
          Serial.println("turn---RIGHT---target: "+(String)targetGive );
      ...
      if(taskProcessing == TURN_LEFT_5)// 10 degree
      {
         ...
         if(directionSense == 'F')
         {
          Serial.println("turn---LEFT---target: "+(String)targetGive );
          target = pos + targetGive;//40
         }
         if(directionSense == 'B')
         {
...
void distanceRide()
{
      ...
      if(taskProcessing == RIDE_1CM)
      {
         if(directionSense == 'F')
         {
            Serial.println("directionRide---FORWARD---target: "+(String)targetGive );
            target = pos + targetGive;
         }
         else if(directionSense == 'B')
            target = pos - 50;
...
void mainFunction()
{
  ...
  speedChecker();
  /*Serial.println("-------------------------------------------");
  Serial.println("task processed: " + (String)taskProcessed);
  Serial.println("task processing: " + (String)taskProcessing);*/
...
void test_backward_move()
{
  numberTest = numberTest + 1;
  
  if(numberTest == 1)
  {
    msg = RIDE_15CM;//RIDE_1CM
  //targetGive = 100*5;
  speedMotor = 51;
  directionSense = 'B';
  Serial.println("test");
  }
 Serial.println((String)numberTest);
  if(numberTest == 1000)
  {
    numberTest = 0;
    //msg = 'B';
  ...
  if((turnSide != 'R')&&(turnSide != 'L'))
  {
    if(directionSense == 'F')
    {
        if( pos > pos_i + 2)
        {
          ...
          if(speedMotor >= 102){
            analogWrite(PWM,(speedMotor/(2*abs(pos - pos_i)))); 
          } 
        }
        if( pos_i > pos + 2)
        {
