...
char directionSense = 'F', turnSide = 'O', msg = ' ';
bool wait = false, taskProcessed = true, infiniteRide = false;
char taskProcessing = NONE;
int target = 0,speedMotor = 0;
...
void initMsg()
{
  ...
  {/*
    // set target position
    msg = mySerial.read() ;//1200;
    //target = 250*sin(prevT/1e6);
    int currentTime = millis();
    if(msg == REQUEST_OPEN)
    {
        mySerial.print(REQUEST_FEEDBACK);
        delay(5);
        while((millis() - currentTime) <= 15)
        {          
          target = mySerial.read();
          mySerial.print(REQUEST_CLOSE);
        }
    }
    */msg = /*my*/Serial.read();
      //Serial.print(msg);
...
void trajectoryCorrection()
{
  ...
  if((turnSide != 'R')&&(turnSide != 'L'))
  {
    if(directionSense == 'F')
    {
        if( pos > pos_i + 16)
        {
          analogWrite(PWM,speedMotor - 25); 
        }
        if( pos_i > pos + 16)
        {
          analogWrite(PWM + 1,speedMotor - 25);
        }
    }
    if(directionSense == 'B')
    {
